...
module HTMLDiff
  class DiffBuilder
    ...
    def operations
      ...
      matches << Match.new(@old_words.size, @new_words.size, 0)
    ...
    def insert_tag(tagname, cssclass, words)
      loop do
        ...
        non_tags = extract_consecutive_words(words) { |word| (Tokenizer.img_tag?(word)) || (!Tokenizer.tag?(word)) }
        @content << wrap_text(non_tags.join, tagname, cssclass) unless non_tags.join.empty?

        break if words.empty?
        @content += extract_consecutive_words(words) { |word| Tokenizer.tag?(word) }
      end
    end
